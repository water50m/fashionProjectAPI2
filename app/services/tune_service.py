        ...
        try:
            ...
            for id, df in gruop_data:
                ...
                try:
                    ...
                    print(f'ID {id} พบข้อมูลไม่ถึง 2 class')
                    return_data = pd.concat([df, return_data], ignore_index=True)
                    continue

                for idx,item in df:
                    if item['class_id'] in body_T_class:
...
def process(path=None,Data=None):
    try:
        ...
        save_path = Path(dir+'tuned_result\\'+'tuned_'+file_readed)

        tuned.to_json(save_path, orient='columns', force_sacii=False, indent=4)
        return True
