...
import torch
import numpy as np
from lib.deepsort.deep_sort_pytorch.utils.parser import get_config
from lib.deepsort.deep_sort_pytorch.deep_sort import DeepSort
...
from lib.deepsort.models.common import DetectMultiBackend
from lib.deepsort.utils.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadScreenshots, LoadStreams
from lib.deepsort.utils.general import (LOGGER, Profile, check_file, check_img_size, check_imshow, check_requirements, colorstr, cv2,
                           increment_path, non_max_suppression, print_args, scale_boxes, strip_optimizer, xyxy2xywh)
from lib.deepsort.utils.plots import Annotator, colors, save_one_box
from lib.deepsort.utils.torch_utils import select_device, smart_inference_mode

def initialize_deepsort():
    # Create the Deep SORT configuration object and load settings from the YAML file
    cfg_deep = get_config()
    cfg_deep.merge_from_file(ROOT/"deep_sort_pytorch/configs/deep_sort.yaml")

    # Initialize the DeepSort tracker
    deepsort = DeepSort((ROOT/cfg_deep.DEEPSORT.REID_OSNET),
...
def classNames(classname):
    ...
    if classname == 'clothing':
        cocoClassNames = ['short sleeve top','long sleeve top','short sleeve outwear','long sleeve outwear',
...
def draw_boxes(frame, bbox_xyxy, draw_trails, className, identities=None, categories=None, offset=(0,0)):
    ...
    for i, box in enumerate(bbox_xyxy):
        ...
        cv2.putText(frame, label, (x1, y1 + 30), 0, 0.5, color, thickness=1, lineType=cv2.LINE_AA)
...
def run(
        weights=ROOT / 'yolov11n.pt',  # model path or triton URL
        source=ROOT / '4p-c0-new.mp4',  # file/dir/URL/glob/screen/0(webcam)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                if len(outputs) > 0:
                    ...
                    object_id = outputs[:, -1]
                    result_pred_clothing.append([bbox_xyxy,identities,ims])
        ...
        if pred_clothing:
            from app.services.predict_service2 import predict_ctrl
            return predict_ctrl(result_pred_clothing,filename,source)
    if update:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov11n.pt', help='model path or triton URL')
    # parser.add_argument('--source', type=str, default=ROOT / 'data/images', help='file/dir/URL/glob/screen/0(webcam)')
    parser.add_argument('--source', type=str, default=ROOT / '4p-c0-new.mp4', help='file/dir/URL/glob/screen/0(webcam)')
