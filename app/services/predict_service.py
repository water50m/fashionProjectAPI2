...
def mark_file_as_processed(filename):
    with open(config.get('PROCESSED_LOG'), 'a') as f:
        f.write(filename + '\n')

def write_log(filename, cfg, predict_id, process_type):
...
def predict_clothing(video_path: str, model_B, output_csv: str, class_selected=None) -> Generator[dict, None, None]:
    ...
    while cap.isOpened():
        ...
        if not success:
            break
        predict_results = model_B.predict(source=frame, verbose=False, conf=conf_thres, iou=0.5, classes=None if not class_selected else class_selected)[0]
        detections = []
        ...
        frame_index = frame_index + 1
    cap.release()
    data_manage.save_json(output_csv, clean_detections_clothing)
    if class_selected is None:
        config.update({'CSV_CLOTHING_RESULT': output_csv})
    else:  # inserted
        config.update({'CSV_CUSTOM_CLOTHING_RESULT': output_csv})
    save_config(config)
...
def upload_video(video_file):
    try:
        video_path = os.path.join(config.get('UPLOAD_VIDEO_PATH', ''), video_file.filename)
...
def start_process_select_detection(model, custom_detection_data, files):
    try:
        model_A = YOLO(config.get('AI_MODEL_PATH', '') + config.get('TRACKING_AI', '')).to('cuda')
        model_B = YOLO(config.get('AI_MODEL_PATH', '') + model).to('cuda')
        dir = config.get('CUSTOM_RESULT_PREDICT_DIR', '')
        output_csv = get_result_csv(dir + 'clothing_detection', True, 'clothing_detection')
        resultpeople_detection_csv = get_result_csv(dir + 'people_detection', True, 'people_detection')
        class_selected = []
        result = []
        cfg = load_config_()
        ...
        files_name = []
        for video in files:
            video_path = upload_video(video)
            filename = video.filename
            print(f'[▶] Processing: {filename}')
            files_name.append(filename)
            if not class_selected:
                detect = detect_objects(video_path, model_A, model_B, output_csv, cfg, resultpeople_detection_csv)
            else:  # inserted
